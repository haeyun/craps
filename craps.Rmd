...
x <- tibble(p_IB = map_int(1:6, ~ roll_two_dice()), 
                  p_JM = map_int(1:6, ~ roll_two_dice())) %>%
  mutate(IB_craps = p_IB %in% c(7,11)) %>%
  mutate(JM_craps = p_JM %in% c(7,11)) %>%
  # mutate(winner = ifelse(IB_craps == TRUE & JM_craps == FALSE, "IB", "Tie"),
  #        winner = ifelse(JM_craps == TRUE & IB_craps == FALSE, "JM", "Tie"))
  mutate(winner = "Tie") %>%
  mutate(winner = ifelse(IB_craps & !JM_craps, "IB", winner),
         winner = ifelse(JM_craps & !IB_craps, "JM", winner))
